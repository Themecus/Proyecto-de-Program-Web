const TOTAL_POKEMON = 150;
        ...
        let mostrarSoloObtenidos = true; // Controla si mostramos solo obtenidos o toda la colección
        let pressTimer; // Para el evento de mantener presionado
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Configurar eventos para el botón limpiar (clic normal y mantener presionado)
            const limpiarBtn = document.getElementById('limpiarFiltros');
            
            limpiarBtn.addEventListener('mousedown', function() {
                // Iniciar temporizador para mantener presionado
                pressTimer = window.setTimeout(function() {
                    mostrarTodaLaColeccion();
                }, 2000); // 2 segundos
            });
            
            limpiarBtn.addEventListener('mouseup', function(e) {
                // Si se soltó antes de 2 segundos, es un clic normal
                clearTimeout(pressTimer);
                
                // Solo ejecutar si no fue un mantenimiento prolongado
                if (e.timeStamp - e.currentTarget.dataset.downTime < 2000) {
                    limpiarFiltros();
                }
            });
            
            limpiarBtn.addEventListener('mouseleave', function() {
                clearTimeout(pressTimer);
            });
            
            limpiarBtn.addEventListener('touchstart', function(e) {
                // Para dispositivos táctiles
                pressTimer = window.setTimeout(function() {
                    mostrarTodaLaColeccion();
                }, 2000);
                e.currentTarget.dataset.downTime = e.timeStamp;
                e.preventDefault(); // Evitar scroll
            });
            
            limpiarBtn.addEventListener('touchend', function(e) {
                clearTimeout(pressTimer);
                if (e.timeStamp - e.currentTarget.dataset.downTime < 2000) {
                    limpiarFiltros();
                }
        ...
        async function inicializarColeccionGrid() {
            ...
            try {
                ...
                renderizarPokemonGrid(todosLosPokemon.filter(pokemon => 
                    !mostrarSoloObtenidos || coleccionPokemon[pokemon.id-1] !== null
                ));
        ...
        function aplicarFiltros() {
            ...
            // Filtrar la lista de Pokémon según lo que estemos mostrando
            const listaAFiltrar = mostrarSoloObtenidos 
                ? todosLosPokemon.filter(pokemon => coleccionPokemon[pokemon.id-1] !== null)
                : todosLosPokemon;
            
            const pokemonFiltrados = listaAFiltrar.filter(pokemon => {
        ...
        function limpiarFiltros() {
            ...
            // Volver al modo de solo mostrar obtenidos
            mostrarSoloObtenidos = true;
            ...
            mostrarNotificacion("Filtros limpiados. Mostrando solo Pokémon obtenidos.");
        }
        
        function mostrarTodaLaColeccion() {
            mostrarSoloObtenidos = false;
            document.getElementById('filtroNombre').value = '';
            document.getElementById('filtroTipo').value = '';
            
            // Mostrar todos los Pokémon (obtenidos y no obtenidos)
            renderizarPokemonGrid(todosLosPokemon);
            mostrarNotificacion("Mostrando toda la colección (obtenidos y no obtenidos)");
        }
